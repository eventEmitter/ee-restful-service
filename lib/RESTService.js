(function(){
	...
	module.exports = new Class( {
		...
		, init: function(options){
			...
			// we shoudl load the controllers from a directy
			if (options.controllers){
				this._loadControllers(options.controllers, options.controllerOptions, function(err){
		...
		/**
		 * the _loadControllers() method load constroller from a direcots
		 *
		 * @param <String> absolute or relative path to the controlers directors
		 * @param <Object> options object for the controllers
		 */
		, _loadControllers: function(path, controllerOptions, callback){
			path = path && path[0] === '/' ? path : path.resolve(project.root+'/', path);
				...
				if (err) callback(err);
					...
					files.forEach(function(fileName){
						if(fileName.substr(fileName.length - 3) === '.js'){
							var controller = new (require(this.dir+'/'+fileName))(controllerOptions);

							controller.urls.forEach(function(url){
								this.use(url, controller);
							}.bind(this));
						}
					}.bind(this));
		...
		, request: function(request, response, next){
			...
			controller = this._hashes.hasOwnProperty(hash) ? this._hashes[hash] : null;
